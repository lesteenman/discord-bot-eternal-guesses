...
class TriggerGuessModalRoute(Route):
    def __init__(self, message_provider: MessageProvider, games_repository: GamesRepository):
        self.games_repository = games_repository
        self.message_provider = message_provider

    async def call(self, event: DiscordEvent) -> DiscordResponse:
        component_action = event.component_action
        game_id = re.search(r"button_trigger_guess_modal_(.*)", component_action.component_custom_id).group(1)

        game = self.games_repository.get(guild_id=event.guild_id, game_id=game_id)
        ...
        return DiscordResponse.modal(
            ...
            components=[
                DiscordComponent.text_input(
                    ...
                    label=self.message_provider.modal_input_label_guess_value(game)
                )
            ]
        )
